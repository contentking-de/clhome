...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const t = await getTranslations("KiSchulungen");
  return buildPageMetadata({
    title: t("metaTitle"),
    description: t("metaDescription"),
    path: "/leistungen/ki-schulungen",
    locale,
  });
}

export default async function KiSchulungenPage() {
  const t = await getTranslations("KiSchulungen");
  ...
  return (
    <SubpageShell>
      ...
      <section style={{ borderBottom: "1px solid var(--line-2)" }}>
        <div className="l-container" style={{ padding: "96px 32px" }}>
          <div className="l-label" style={{ marginBottom: 18 }}>{t("modulesLabel")}</div>
